import { Button, CustomSelect, InputFile, InputForm, Map, Select, TextField, Title } from "@/components"
import React, { useEffect, useState } from "react"
import { useForm } from "react-hook-form"
import withBaseTopping from "@/hocs/withBaseTopping"
import { getDataVn } from "@/redux/actions"
...
const UpdatePost = ({ dispatch, editPost, rend1er }) => {
  ...
  // Hàm setValue cho các fields
  const setCustomValue = (id, val) =>
    setValue(id, val, { shouldDirty: true, shouldTouch: true, shouldValidate: true })
  useEffect(() => {
    // Lấy full tỉnh thành Việt Nam
    dispatch(getDataVn())
  }, [])
  // Nếu chọn tỉnh thì sẽ reset quận huyện xã phường
  useEffect(() => {
    if (province) {
  ...
  useEffect(() => {
    setCustomValue("address", editPost?.address)
    setCustomValue("category", editPost?.category)
    setCustomValue("title", editPost?.title)
    setCustomValue("description", editPost?.description)
    setCustomValue("price", editPost?.price)
    setCustomValue("area", editPost?.area)
    setCustomValue("target", editPost?.target)
    setCustomValue("images", editPost?.images)
    setCustomValue("isAvailable", editPost?.isAvailable ? 1 : 0)
  }, [editPost])
  // Handle Submit Form
  ...
  // console.log({ target, category, categories })
  return (
    <section
    ...
    >
      <div className="p-4 flex items-center justify-between border-b">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button
            onClick={() => dispatch(modal({ isShowModal: false, modalChildren: null }))}
      ...
      <section className="p-4 grid grid-cols-12 gap-6">
        <div className="col-span-8">
          ...
          <div className="mt-6 relative z-10">
            <Select
              options={categories?.map((el) => ({ ...el, value: el.id, label: el.value, name: el.value }))}
          ...
          <CustomSelect
            ...
            validate={{ required: "Trường này không được bỏ trống." }}
            options={[
              { value: 1, text: "Chưa thuê" },
              { value: 0, text: "Đã thuê" },
            ]}
        ...
        <div className="col-span-4 flex flex-col gap-4">
          <div className="w-full relative z-10 h-[300px]">
            {center && <Map address={address} zoom={zoom} />}
          </div>
...
export default withBaseTopping(UpdatePost)
